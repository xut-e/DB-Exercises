...
SELECT USERS.user, USERS.host, PRIVILEGIOS.table_name, PRIVILEGIOS.privilege_type FROM mysql.user AS USERS JOIN mysql.db AS DB ON USERS.user = DB.user AND USERS.host = DB.host JOIN mysql.tables_priv AS PRIVILEGIOS ON DB.db = PRIVILEGIOS.db WHERE DB.db = 'galaxia';
...
SELECT column_name, data_type, is_nullable FROM information_schema.tables WHERE table_schema = 'galaxia' AND table_name = 'TRIPULANTES';